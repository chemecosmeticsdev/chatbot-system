...
export const ChatbotInstanceSchema = z.object({
  ...
  retrieval_k: z.number().int().min(1).max(20).default(5),
  score_threshold: z.number().min(0).max(1).default(0.7),
  context_window: z.number().int().min(1000).max(32000).default(4000),
...
export const CreateChatbotInstanceSchema = z.object({
  ...
  llm_model: z.string().min(1),
  system_prompt: z.string().min(10).max(8000),
  model_config: z.record(z.any()).optional().default({}),
  rag_enabled: z.boolean().optional().default(true),
  retrieval_k: z.number().int().min(1).max(20).optional().default(5),
  score_threshold: z.number().min(0).max(1).optional().default(0.7),
  context_window: z.number().int().min(1000).max(32000).optional().default(4000),
  welcome_message: z.string().max(1000).optional(),
  fallback_message: z.string().max(1000).optional()
});
